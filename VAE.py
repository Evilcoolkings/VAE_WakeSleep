...
def visulazation(sess, X):
    ...
    plt.legend()
    plt.grid(True)
    plt.title("VAE_Z_distribution")
    #plt.show()
    fig.savefig("VAE_Z_distribution.png")
    ...
    fig = plot(samples,20)
    # plt.show()
    fig.savefig("VAE_X_distribution.png")
...
def main():
    ...
    print("save_model in: "+str(save_path))


    saver.restore(sess, "Model/vae_model.ckpt")
